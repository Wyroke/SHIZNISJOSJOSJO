...
local loadstring = function(...)
	...
	if err and vape then
		vape:CreateNotification('SpecV4', 'Failed to load : '..err, 30, 'alert')
	end
	return res
end
		...
		if (not teleportedServers) and (not shared.VapeIndependent) then
			...
			local teleportScript =[[
				...
				if shared.VapeDeveloper then
					...
					loadstring(game:HttpGet('https://raw.githubusercontent.com/Wyroke/SHIZNISJOSJOSJO/'..readfile('ReVape/profiles/commit.txt')..'/loader.lua', true), 'loader')()
				end
			]]
			if shared.VapeDeveloper then
...
if not shared.VapeIndependent then
	...
	if isfile('ReVape/games/'..game.PlaceId..'.lua') then
		...
		if not shared.VapeDeveloper then
			local suc, res = pcall(function()
				return game:HttpGet('https://raw.githubusercontent.com/Wyroke/SHIZNISJOSJOSJO/'..readfile('ReVape/profiles/commit.txt')..'/games/'..game.PlaceId..'.lua', true)
			end)
